...
export const authInstance  = axios.create({ ...baseConfig, withCredentials: true })
```
...
한 가지 더, `isRefreshing`(axios 레이어)과 SSE 레이어의 재발급 락은 서로 독립적입니다. axios 401과 SSE 401이 동시에 발생하면 두 레이어가 각자 재발급을 시도합니다. 이를 근본적으로 해결하려면 재발급 로직을 공유 모듈로 추출해 두 레이어가 같은 Promise를 바라보게 해야 합니다. 이 문제는 `sseRefreshPromise` 싱글턴 패턴으로 별도 해결했습니다. 같은 원리를 fetch 레이어에서 재구현한 이야기는 다음 글에서 이어집니다.
